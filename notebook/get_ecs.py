...
def analyze_ecs_cpu(normal_start, normal_end, Target_ECS, show):
    ...
    query_template = f"""
        .entity_set with(domain='acs', name='acs.ecs.instance', query=`instance_id='{Target_ECS}'`)
        | entity-call get_metric('acs', 'acs.metric.prometheus_ecs_high_level_metric_node', 'aggregate_node_cpu_usage', 'range', '1m')
        """
    ...
    return is_anomaly, max_cpu

def analyze_ecs_memory(normal_start, normal_end, Target_ECS, show):
    # 1. 计算三个时段的时间戳（转为int类型，CMS查询要求）
    # 前10分钟：normal_start - 10min 到 normal_start
    pre10_start = int((normal_start - timedelta(minutes=10)).timestamp())
    pre10_end = int(normal_start.timestamp())
    ...
    query_template = f"""
        .entity_set with(domain='acs', name='acs.ecs.instance', query=`instance_id='{Target_ECS}'`)
        | entity-call get_metric('acs', 'acs.metric.prometheus_ecs_high_level_metric_node', 'aggregate_node_memory_usage', 'range', '1m')
        """
    ...
    return is_anomaly, max_memory

def analyze_ecs_disk(normal_start, normal_end, Target_ECS, show):
    # 1. 计算三个时段的时间戳（转为int类型，CMS查询要求）
    # 前10分钟：normal_start - 10min 到 normal_start
    pre10_start = int((normal_start - timedelta(minutes=10)).timestamp())
    pre10_end = int(normal_start.timestamp())
    ...
    query_template = f"""
        .entity_set with(domain='acs', name='acs.ecs.instance', query=`instance_id='{Target_ECS}'`)
        | entity-call get_metric('acs', 'acs.metric.prometheus_ecs_high_level_metric_node', 'aggregate_node_disk_usage', 'range', '1m')
        """
    ...
    timestamps, disk = get_result(result)
    ...
    normal_end_dt = datetime.fromtimestamp(normal_end_ts)
    pre_values, normal_values, post_values = split_time_period_data(
        timestamps, disk, pre10_end_dt, normal_end_dt
    )
    ...
    print(f"检测时段平均值: {normal_avg:.4f}")
    print(f"后10分钟平均值: {post_avg:.4f}")
    print(f"最大CPU使用率: {max_disk:.4f}")

    if is_anomaly:
        print(f"🔴 异常检测: 检测时段disk明显高于前后时段!")
    else:
        print(f"🟢 异常检测: 检测时段disk处于正常范围")

    if show:
        plt.figure(figsize=(12, 6))

        plt.plot(timestamps, disk, marker='o', linestyle='-', color='b')
        pre10_start_dt = datetime.fromtimestamp(pre10_start)  # 前10分钟开始（datetime）
        pre10_end_dt = datetime.fromtimestamp(pre10_end)  # 前10分钟结束（datetime）
        normal_start_dt = datetime.fromtimestamp(normal_start_ts)  # 目标时段开始（datetime）
        normal_end_dt = datetime.fromtimestamp(normal_end_ts)  # 目标时段结束（datetime）
...
if __name__ == "__main__":
    ...
    for problem_data in input_data:
        if problem_data.get("problem_id") == problem_id:
            ...
            target_service = "i-m5ec00yjg8kxv34hyr0k"
            show = True
            # cpu_anomaly = analyze_ecs_cpu(normal_start, normal_end, target_service, show)
            # memory_anomaly = analyze_ecs_memory(normal_start, normal_end, target_service, show)
            disk_anomaly, max_disk = analyze_ecs_disk(normal_start, normal_end, target_service, show)
            # cpu_list = []
            # memory_list = []
            ...
            #         cpu_anomaly = analyze_cpu(normal_start, normal_end, service, show)
            #         if cpu_anomaly:
            #             cpu_list.append(service)
            #         # 5. 查询Memory数据
            #         print(f"🔍 查询 {service} 服务Memory数据...")
            #         memory_anomaly = analyze_memory(normal_start, normal_end, service, show)
            #         if memory_anomaly:
            #             memory_list.append(service)
